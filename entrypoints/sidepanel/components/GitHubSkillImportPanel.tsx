...
export default function GitHubSkillImportPanel({ onImported, onCancel }: Props) {
  ...
  const runPreview = async () => {
    ...
    try {
      ...
      if (!granted) throw new Error(t.ghImportNeedsPermission);
      const response = await chrome.runtime.sendMessage({
        type: 'PREVIEW_GITHUB_SKILL_SOURCE',
        payload: { url: requestedUrl },
      });
      if (response?.ok === false) throw new Error(response.error ?? t.ghImportPreviewFailed);
  ...
  const runImport = async () => {
    ...
    try {
      ...
      if (response?.ok === false) throw new Error(response.error ?? t.ghImportFailed);
      const importResult = response as GitHubSkillImportResult;
      setResult(importResult);
      setState('success');
      setMessage(t.ghImportSuccess.replace('{n}', String(importResult.imported.length)));
      await onImported();
    } catch (error) {
      setState('error');
      setMessage(error instanceof Error ? error.message : String(error));
    ...
    <section className="ds-form rounded-xl p-4 space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          <input
            type="url"
            placeholder="https://github.com/owner/repo or .../SKILL.md"
            value={url}
            onChange={(event) => {
          ...
          >
            ...
            {state === 'previewing' ? t.ghImportPreviewing : t.ghImportPreview}
          </button>
        </div>
      ...
      {preview && (
        <div className="space-y-3">
          <SourceSummary preview={preview} openRepoLabel={t.ghImportOpenRepo} />
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            >
              {allSelected ? t.ghImportDeselectAll : t.ghImportSelectAll}
            </button>
            <span className="text-[11px]" style={{ color: 'var(--ds-text-tertiary)' }}>
              {t.ghImportSelected
                .replace('{n}', String(selectedCount))
                .replace('{total}', String(preview.skills.length))
          ...
          <div className="space-y-2">
            {preview.skills.map((skill) => (
              <PreviewSkillRow
                ...
                renamedLabel={t.ghImportSkillRenamed}
              />
            ))}
          </div>
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            >
              ...
              {state === 'importing' ? t.ghImportImporting : t.ghImportConfirm}
            </button>
          </div>
        </div>
      )}
      ...
      {message && (
        <StatusMessage state={state} message={message} result={result} renamedTemplate={t.ghImportRenamed} />
      )}
    </section>
  );
}

function SourceSummary({ preview, openRepoLabel }: { preview: GitHubSkillPreview; openRepoLabel: string }) {
  ...
  return (
    <div className="ds-surface-panel rounded-xl p-3 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        ...
        <Meta label="Skills" value={String(preview.skills.length)} />
        <Meta label="Default branch" value={source.defaultBranch} />
      </div>
      {warnings.length > 0 && (
        <div className="rounded-lg px-3 py-2 text-[11px] leading-relaxed" style={{ color: 'var(--ds-warning)', background: 'var(--ds-warning-bg)' }}>
          ...
          {warnings.length > 4 && <div>• +{warnings.length - 4} more warnings</div>}
        </div>
      )}
    </div>
  );
...
function PreviewSkillRow({ skill, checked, onToggle, renamedLabel }: {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2 text-[10px]" style={{ color: 'var(--ds-text-tertiary)' }}>
            ...
            <span className="ds-tag px-1.5 py-0.5 rounded-full">files: {skill.includedFiles.length}</span>
            {skill.omittedFiles.length > 0 && (
              <span className="ds-tag px-1.5 py-0.5 rounded-full">omitted: {skill.omittedFiles.length}</span>
            )}
          </div>
        </div>
      </div>
...
function StatusMessage({ state, message, result, renamedTemplate }: {
  state: ImportState;
  message: string;
  ...
  return (
    ...
    >
      ...
      {result && result.renamed > 0 && (
        <div>{renamedTemplate.replace('{n}', String(result.renamed))}</div>
      )}
    </div>
  );
}
