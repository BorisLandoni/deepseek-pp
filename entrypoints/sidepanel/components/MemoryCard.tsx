...
import { getMemoryTypeMap, SVG_PATHS } from '../constants';
...
export default function MemoryCard({ memory, onDelete, onEdit, onTogglePin }: Props) {
  const { t } = useLanguage();
  const typeMap = getMemoryTypeMap(t);
  const typeInfo = typeMap[memory.type] ?? typeMap.topic;
    ...
    <div className="ds-card rounded-xl p-3.5 group animate-fade-in">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button onClick={onTogglePin} className="ds-action-btn ds-action-btn-pin p-1.5 rounded-md" title={t.memCardPin}>
          ...
          <button onClick={onEdit} className="ds-action-btn ds-action-btn-edit p-1.5 rounded-md" title={t.memCardEdit}>
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d={SVG_PATHS.edit} />
            </svg>
          </button>
          <button onClick={onDelete} className="ds-action-btn ds-action-btn-delete p-1.5 rounded-md" title={t.memCardDelete}>
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
function formatAge(ts: number, t: ReturnType<typeof useLanguage>['t']): string {
