...
export default function SkillForm({ initialSkill, onSave, onCancel }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="ds-form rounded-xl p-4 space-y-3">
      <div>
        ...
        {normalizedName && (
          <p className="text-[11px] mt-1" style={{ color: 'var(--ds-text-tertiary)' }}>
            {t.skillFormTriggerHint}<code className="font-mono" style={{ color: 'var(--ds-blue)' }}>/{normalizedName}</code>
          </p>
        )}
      </div>
      ...
      <input
        type="text"
        placeholder={t.skillFormDescPlaceholder}
        value={description}
      ...
      <div>
        <label className="text-[11px] mb-1.5 block font-medium" style={{ color: 'var(--ds-text-tertiary)' }}>
          {t.skillFormInstructionsLabel}
        </label>
        <textarea
          rows={6}
          placeholder={t.skillFormInstructionsPlaceholder}
          value={instructions}
      ...
      <div className="flex gap-2 justify-end pt-1">
        ...
        >
          {isEditing ? t.skillFormSaveChanges : t.save}
        </button>
      </div>
    </form>
  );
