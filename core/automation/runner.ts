...
function buildAutomationToolContinuationPrompt(executions: ToolExecutionRecord[]): string {
  ...
  return [
    'The following are the MCP tool results just executed by the automation task. Please continue completing the automation task based on these results.',
    'If the results are sufficient, output the final conclusion; only continue calling tools when more information is truly needed.',
    '',
    '<tool_results>',
