...
// These states periodically cycle through lines on long stays; other states only say one line on entry.
...
let petLanguage: Language = 'en';

// Read language preference from storage and keep it in sync
chrome.storage.local.get(['deepseek_pp_language']).then((data) => {
  if (data.deepseek_pp_language === 'it' || data.deepseek_pp_language === 'en') {
    petLanguage = data.deepseek_pp_language as Language;
  }
});
chrome.storage.onChanged.addListener((changes) => {
  if ('deepseek_pp_language' in changes) {
    const val = changes.deepseek_pp_language.newValue;
    if (val === 'it' || val === 'en') petLanguage = val;
  }
});
...
/** After the main world captures DeepSeek request headers, the isolated world reads the token from localStorage and persists it to chrome.storage */
async function persistDeepSeekClientHeaders(): Promise<void> {
  try {
    ...
    if (headers) {
      ...
      // Notify sidebar to re-check login state
      chrome.runtime.sendMessage({ type: 'AUTH_STATUS_CHANGED' }).catch(() => {});
    }
  } catch {
    // Fail silently; will retry when user sends the next message
  }
}
...
function stopInlineAgent(): void {
  ...
  if (container) {
    const footer = createAgentFooter(0, 0, false, 'Stopped');
...
function handleAgentStepComplete(msg: InlineAgentStepCompleteMsg): void {
  ...
  const label = msg.toolExecutions.length > 0
    ? `Complete (${msg.toolExecutions.length} tools)`
    : 'Complete';
...
function renderTokenSpeedIndicator(progress: ResponseTokenSpeedPayload): boolean {
  ...
  badge.setAttribute('title', `Token output speed: ${speed}${progress.active ? '' : ' (idle)'}`);
  return true;
}
...
async function executeToolCall(call: ToolCall): Promise<ToolCardResult> {
  ...
  if (!extensionContextValid) {
    return { ok: false, summary: 'Execution failed', detail: 'Extension reloaded. Please refresh the DeepSeek page and try again.' };
  }
...
async function sendRuntimeToolCallMessage(call: ToolCall): Promise<unknown> {
  ...
  try {
    ...
    return {
      ok: false,
      summary: 'Tool message send failed',
      detail,
      error: {
...
function createInvalidRuntimeToolResult(value: unknown): ToolCardResult {
  ...
  return {
    ...
    detail: missing
      ? 'Background did not return tool execution results. Please refresh the DeepSeek page and try again; if it still fails, re-test Shell Local in the MCP page.'
      : `Invalid tool result structure from background: ${previewUnknown(value)}`,
    error: {
...
function createPermissionBanner(origin: string): HTMLElement | null {
  ...
  banner.innerHTML = `
    <span class="dpp-permission-text">DeepSeek++ needs permission to access <strong>${escapeHtml(origin)}</strong> to fetch this page</span>
...
// --- Tool execution collapsible block (matches official thinking block style) ---
...
function updateToolBlockContent(block: HTMLElement, executions: ToolExecutionRecord[]) {
  ...
  title.textContent = `Tools executed (${count})`;
...
function createRestoredInlineAgentContainer(trace: InlineAgentTraceRecord): HTMLElement {
  ...
  if (trace.status === 'complete') {
    ...
    container.appendChild(createAgentFooter(trace.totalSteps, trace.totalTools, false, trace.error ?? 'Stopped'));
  }
...
function getInlineAgentStepStatusLabel(step: InlineAgentTraceStepRecord): string {
  if (step.status === 'complete') {
    return step.toolExecutions.length > 0
      ? `Complete (${step.toolExecutions.length} tools)`
      : 'Complete';
  }
  if (step.status === 'executing_tools') return 'Executing tools';
...
function summarizeRestoredToolCall(call: ToolCall): ToolCardResult {
  ...
  switch (call.name) {
    case 'memory_save':
      return { ok: true, summary: 'Saved', detail };
    case 'memory_update':
      return { ok: true, summary: 'Updated', detail };
    case 'memory_delete':
      return { ok: true, summary: 'Deleted', detail };
    case 'web_search':
      return { ok: true, summary: 'Searched', detail: String(typeof call.payload.query === 'string' ? call.payload.query : '') };
    case 'web_fetch':
      return { ok: true, summary: 'Fetched', detail: String(typeof call.payload.url === 'string' ? call.payload.url : '') };
    default:
      return { ok: true, summary: 'Executed', detail };
  }
}
...
// The sole exit point for all state transitions: writes dataset and shows a line when the state actually changes.
// (updatePetFromTokenSpeed calls setPetState at high frequency during streaming;
//  change detection here prevents the bubble from being reset repeatedly.)
...
// Show a line when entering a state; schedule the next line for looping states.
function triggerPetBubble(state: PetState) {
  if (!currentPetConfig?.enabled || !petHostEl?.isConnected) return;
  if (petDragState) return; // Stay silent while dragging
  clearPetBubbleRepeatTimer();
  petBubbleState = state;
  showPetBubble(pickPetLine(state, petLanguage, petRecentLines));
...
function armPetBubbleRepeat() {
  ...
  petBubbleRepeatTimer = setTimeout(() => {
    ...
    if (petHostEl.dataset.state !== state) return; // State has changed; let the new state take over
    showPetBubble(pickPetLine(state, petLanguage, petRecentLines));
    armPetBubbleRepeat();
  }, delay);
}
...
// Immediately hide the bubble and stop looping (called on drag / hover / removal).
...
// Hide bubble when mouse enters pet, making room for hover interaction and avoiding obstruction.
...
function finishPetDrag(event: PointerEvent) {
  ...
  // After drag ends, reschedule lines for looping states (silenced by hidePetBubble during drag).
