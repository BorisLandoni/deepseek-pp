...
function replaceMatchWithSummary(match: string, catalog: ToolInvocationCatalog): string {
  ...
  const lines = calls.map(call => {
    ...
    if (call.parseError) return `• ${getToolInvocationLabel(name, catalog)}: format error`;
  ...
  const header = executedCount === calls.length
    ? `🔧 Tools executed (${calls.length})`
    : `🔧 Tools executed (${executedCount}, ${calls.length - executedCount} format error(s))`;
  return '\n\n---\n' + header + '\n' + lines.join('\n') + '\n---';
}
