...
export default function SkillPage() {
  ...
  const handleCheckSource = async (source: GitHubSkillSource) => {
    setSourceActions((current) => ({
      ...current,
      [source.id]: { status: 'checking', message: t.skillChecking },
    }));
    try {
      const granted = await requestGitHubApiPermission();
      if (!granted) throw new Error(t.skillNeedsGitHubPermission);
      const response = await chrome.runtime.sendMessage({
        type: 'CHECK_GITHUB_SKILL_SOURCE_UPDATES',
        payload: { sourceId: source.id },
      });
      if (response?.ok === false) throw new Error(response.error ?? t.skillCheckError);
  ...
  const handleUpdateSource = async (source: GitHubSkillSource) => {
    setSourceActions((current) => ({
      ...current,
      [source.id]: { status: 'updating', message: t.skillSyncing },
    }));
    try {
      const granted = await requestGitHubApiPermission();
      if (!granted) throw new Error(t.skillNeedsGitHubPermission);
      ...
      if (response?.ok === false) throw new Error(response.error ?? t.skillSyncError);
      setSourceActions((current) => ({
        ...current,
        [source.id]: {
          status: 'success',
          message: t.ghImportSuccess.replace('{n}', String((response?.imported as Skill[] | undefined)?.length ?? source.skillPaths.length)),
        },
      }));
      await load();
    } catch (error) {
  ...
  const handleDeleteSource = async (source: GitHubSkillSource) => {
    if (!confirm(t.skillDeleteSourceConfirm.replace('{n}', String(source.importedSkillNames.length)).replace('{repo}', source.repository))) return;
    await chrome.runtime.sendMessage({
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <SkillSection title={t.skillRemote} skills={remote} onDelete={handleDelete} onToggleEnabled={handleToggleEnabled} />
      <SkillSection title={t.skillBuiltin} skills={builtin} />
      <SkillSection title={t.skillOfficial} skills={official} />
      <SkillSection title={t.skillCustom} skills={custom} onEdit={handleEdit} onDelete={handleDelete} onToggleEnabled={handleToggleEnabled} />

      <div className="ds-info-panel rounded-xl p-3.5">
        <p className="text-xs leading-relaxed" style={{ color: 'var(--ds-text-secondary)' }}>
          {t.skillHint.split('/frontend-design').map((part, i) =>
            i === 0 ? part : (
              <span key={i}>
                <code className="ds-code font-mono text-[11px] px-1.5 py-0.5 rounded">/frontend-design</code>
                {part}
              </span>
            )
          )}
    ...
    <div className="ds-surface-panel rounded-xl p-3.5 space-y-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button type="button" onClick={onCheck} disabled={busy} className="ds-btn-secondary px-2 py-1 text-[11px] font-medium rounded-md disabled:opacity-40">
            {t.skillCheck}
          </button>
          <button type="button" onClick={onUpdate} disabled={busy} className="ds-btn-secondary px-2 py-1 text-[11px] font-medium rounded-md disabled:opacity-40">
            {t.skillSync}
          </button>
          <button type="button" onClick={onDelete} disabled={busy} className="ds-text-btn-delete px-2 py-1 text-[11px] font-medium rounded-md disabled:opacity-40">
            {t.skillRemove}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 text-[10px]" style={{ color: 'var(--ds-text-tertiary)' }}>
        <span className="ds-tag px-1.5 py-0.5 rounded-full">{t.skillGitHubImported.replace('{n}', String(source.importedSkillNames.length))}</span>
        ...
        <span className="ds-tag px-1.5 py-0.5 rounded-full">{t.skillSync} {formatTime(source.updatedAt)}</span>
        {source.lastCheckedAt && <span className="ds-tag px-1.5 py-0.5 rounded-full">{t.skillCheck} {formatTime(source.lastCheckedAt)}</span>}
      </div>
...
function formatUpdateMessage(update: GitHubSkillUpdatePreview, t: ReturnType<typeof useLanguage>['t']): string {
  if (!update.hasUpdates) return t.skillNoUpdates;
  const parts: string[] = [];
  if (update.changedPaths.length > 0) parts.push(`${update.changedPaths.length} imported Skills may have updates`);
  if (update.newPaths.length > 0) parts.push(`${update.newPaths.length} new Skills`);
  if (update.missingPaths.length > 0) parts.push(`${update.missingPaths.length} imported Skills removed upstream`);
  return parts.join(', ') || t.skillUpdatesFound;
}
