...
export async function importGitHubSkillSource(
  ...
  if (request.selectedPaths.length === 0) {
    throw new Error('Please select at least one Skill before importing');
  }
  ...
  if (missingPaths.length > 0) {
    throw new Error(`Selected Skill paths do not exist in the GitHub source: ${missingPaths.join(', ')}`);
  }
  if (selected.length === 0) {
    throw new Error('Selected Skill paths do not exist in the GitHub source');
  }
...
export async function checkGitHubSkillSourceUpdates(sourceId: string): Promise<GitHubSkillUpdatePreview> {
  ...
  if (!source) throw new Error('GitHub Skill source not found');
...
export async function updateGitHubSkillSource(sourceId: string): Promise<GitHubSkillImportResult> {
  ...
  if (selectedPaths.length === 0) {
    throw new Error('The upstream no longer contains any Skills currently imported from this source; update stopped to avoid clearing local content');
  }
...
async function loadGitHubSkillSource(url: string, selectedPaths?: Set<string>): Promise<LoadedGitHubSource> {
  ...
  if (tree.truncated) {
    warnings.push('The repository tree returned by GitHub was truncated; some Skill files may be missing');
  }
  if (skillPaths.length === 0) {
    throw new Error('No SKILL.md found under this GitHub link');
  }
  if (skillPaths.length > MAX_SKILLS_PER_SOURCE) {
    warnings.push(`Found ${skillPaths.length} Skills; only the first ${MAX_SKILLS_PER_SOURCE} are previewed`);
  }
...
async function loadGitHubSkill(
  ...
  if (content.length > MAX_SKILL_BYTES) {
    throw new Error(`${skillPath} is too large, import stopped (${content.length} bytes)`);
  }
...
function parseGitHubUrl(input: string): ParsedGitHubUrl {
  ...
  if (!trimmed) throw new Error('GitHub link cannot be empty');
  ...
  try {
    ...
    throw new Error('Please enter a GitHub repository, directory, or SKILL.md link');
  }

  if (url.hostname === 'raw.githubusercontent.com') {
    const [owner, repo, ...refPathParts] = url.pathname.split('/').filter(Boolean);
    if (!owner || !repo || refPathParts.length < 2) throw new Error('Raw GitHub link is missing repository or path');
  ...
  if (url.hostname !== 'github.com') {
    throw new Error('Only github.com or raw.githubusercontent.com links are currently supported');
  }

  const [owner, rawRepo, action, ...rest] = url.pathname.split('/').filter(Boolean);
  if (!owner || !rawRepo) throw new Error('GitHub link is missing owner/repo');
  const repo = stripGitSuffix(rawRepo);

  if (action === 'tree' || action === 'blob') {
    if (rest.length === 0) throw new Error('GitHub tree/blob link is missing a branch');
    return {
      owner,
      repo,
      mode: action,
...
async function resolveSourceLocation(
  ...
  throw new Error('The branch, tag, or commit in the GitHub link does not exist');
}
...
function findSkillPaths(tree: GitHubTreeResponse, rootPath: string, mode: ParsedGitHubUrl['mode']): string[] {
  ...
  if (mode === 'blob') {
    if (!normalizedRoot.endsWith('SKILL.md')) throw new Error('Single-file import only supports SKILL.md');
    const exists = tree.tree.some((entry) => entry.type === 'blob' && entry.path === normalizedRoot);
    if (!exists) throw new Error(`${normalizedRoot} does not exist in the GitHub source`);
...
async function fetchResourceBundle(
  ...
  if (omitted.length > 0) {
    warnings.push(`${omitted.length} same-directory resource(s) were not bundled; see the upstream repository for details`);
  }
...
function buildImportedInstructions(input: {
  ...
  const resourceDocs = resources.included.length === 0 ? '' : [
    ...
    '',
    'These files come from the same upstream Skill directory and supplement the agents, references, templates, or examples referenced in the original SKILL.md.',
    '',
  ...
  const omitted = resources.omitted.length === 0 ? '' : [
    ...
    '',
    'The following files were not bundled into the prompt due to count or size limits; refer to the upstream repository if needed.',
    '',
...
async function fetchPackageInfo(owner: string, repo: string, ref: string): Promise<{ version?: string; description?: string }> {
  for (const path of ['.codex-plugin/plugin.json', '.claude-plugin/plugin.json', 'package.json']) {
    ...
    try {
      ...
      throw new Error(`${path} is not valid JSON; import stopped`);
    }
    return {
...
async function fetchGitHubJson<T>(path: string): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`GitHub request failed (HTTP ${response.status})${detail ? `: ${detail.slice(0, 180)}` : ''}`);
    }
    ...
    if (error instanceof TypeError) {
      throw new Error('Cannot access the GitHub API; please grant GitHub access permission and confirm the network is available');
    }
    throw error;
  } finally {
...
function normalizeSkillName(name: string): string {
  ...
  if (!normalized) throw new Error('GitHub Skill is missing a valid name');
  return normalized;
}
...
function createUniqueSkillName(preferred: string, occupiedNames: Set<string>): string {
  ...
  throw new Error(`Cannot generate a unique name for remote Skill: ${preferred}`);
}
