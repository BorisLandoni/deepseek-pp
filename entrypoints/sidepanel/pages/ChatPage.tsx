...
export default function ChatPage() {
  ...
  // Drag & drop files anywhere on the chat to attach them (images/PDF/text), reusing the same
  // handler as the Attach button. A depth counter avoids flicker as the pointer crosses children.
  const handleChatDragEnter = (e: ReactDragEvent) => {
  ...
  const handleChatDragLeave = (_e: ReactDragEvent) => {
  ...
  return (
    ...
    >
      {isDragging && (
        <div
          className="absolute inset-0 z-30 m-1 flex items-center justify-center pointer-events-none rounded-xl"
          style={{ background: 'var(--ds-blue-light, rgba(59,130,246,0.14))', border: '2px dashed var(--ds-blue, #3b82f6)' }}
        >
          <div className="flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--ds-blue, #3b82f6)' }}>
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
            </svg>
            {language === 'it' ? 'Rilascia qui per allegare' : 'Drop here to attach'}
          </div>
        </div>
      )}
      {/* Header */}
      ...
      {/* Page tools: summarize current page · summary language · ask-about-page toggle · attach.
          Larger, evenly-styled chips; the toggle uses a solid fill + check so its ON state is
          unmistakable. Files can also be dragged & dropped anywhere on the chat. */}
      <div className="px-3 pt-2 flex items-center gap-2 flex-wrap" style={{ borderTop: '1px solid var(--ds-border)' }}>
        <button
          onClick={handleSummarize}
          disabled={isStreaming}
          className="px-3 py-1.5 text-xs font-medium rounded-lg flex items-center gap-1.5 border transition-colors disabled:opacity-40 hover:opacity-80"
          style={{ background: 'var(--ds-surface)', color: 'var(--ds-text-secondary)', borderColor: 'var(--ds-border)' }}
        ...
        {/* Summary language — bordered chip with a globe icon so it clearly reads as a choice */}
        <label
          className="flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 text-xs rounded-lg border cursor-pointer"
          style={{ background: 'var(--ds-surface)', color: 'var(--ds-text-secondary)', borderColor: 'var(--ds-border)' }}
        ...
        >
          <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <path strokeLinecap="round" d="M12 3c2.8 2.4 4 5.6 4 9s-1.2 6.6-4 9c-2.8-2.4-4-5.6-4-9s1.2-6.6 4-9z" />
          </svg>
          <select
            value={summaryLang}
        ...
        {/* Ask-about-page toggle — solid accent fill + check icon when ON = unmistakable selected state */}
        <button
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs font-medium rounded-lg flex items-center gap-1.5 border transition-colors disabled:opacity-40 hover:opacity-80"
          style={{ background: 'var(--ds-surface)', color: 'var(--ds-text-secondary)', borderColor: 'var(--ds-border)' }}
          title={language === 'it' ? 'Allega file (testo, PDF, immagini) — puoi anche trascinarli nella chat' : 'Attach files (text, PDF, images) — you can also drag them into the chat'}
        >
          <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
          </svg>
