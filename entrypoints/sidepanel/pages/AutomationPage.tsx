...
const DEFAULT_TIMEZONE = Intl.DateTimeFormat().resolvedOptions().timeZone || 'Europe/Rome';
...
export default function AutomationPage() {
  ...
  const save = async () => {
    ...
    if (!payload.name || !payload.prompt) {
      setMessage(t.autoValidationNamePrompt);
      return;
    }
    if (payload.schedule.enabled && !payload.schedule.expression) {
      setMessage(t.autoValidationExpression);
      return;
    }
  ...
  const runNow = async (id: string) => {
    ...
    try {
      ...
      if (run && 'error' in run && typeof run.error === 'string') {
        ...
        setMessage(run.error?.message ?? t.autoRunFailed);
      }
    } finally {
      setRunningIds((prev) => {
  ...
  const remove = async (automation: Automation) => {
    if (!confirm(t.autoDeleteConfirm.replace('{name}', automation.name))) return;
    ...
    <div className="p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <div className="text-[11px] mt-0.5" style={{ color: 'var(--ds-text-tertiary)' }}>
            {t.autoTaskCount.replace('{total}', String(automations.length)).replace('{active}', String(activeCount))}
          </div>
        </div>
        <button
          onClick={startCreate}
      ...
      {automations.length === 0 && !showForm ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm" style={{ color: 'var(--ds-text-tertiary)' }}>{t.autoEmpty}</p>
        </div>
      ) : (
    ...
    <div className="ds-form rounded-xl p-4 space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <label className="space-y-1">
          <span className="text-[11px]" style={{ color: 'var(--ds-text-tertiary)' }}>{t.autoFormName}</span>
          <input
            value={form.name}
            onChange={(e) => update('name', e.target.value)}
            className="ds-input w-full px-3 py-2 text-xs rounded-lg"
            placeholder={t.autoFormNamePlaceholder}
          />
        </label>
        <label className="space-y-1">
          <span className="text-[11px]" style={{ color: 'var(--ds-text-tertiary)' }}>{t.autoFormModel}</span>
          <select
            value={form.modelType}
            onChange={(e) => update('modelType', e.target.value)}
            className="ds-input w-full px-3 py-2 text-xs rounded-lg"
          >
            <option value="">{t.autoFormModelDefault}</option>
      ...
      <label className="space-y-1 block">
        <span className="text-[11px]" style={{ color: 'var(--ds-text-tertiary)' }}>{t.autoFormPromptLabel}</span>
        <textarea
          value={form.prompt}
          onChange={(e) => update('prompt', e.target.value)}
          className="ds-input w-full px-3 py-2 text-xs rounded-lg min-h-28 resize-y"
          placeholder={t.autoFormPromptPlaceholder}
        />
      </label>

      <div className="grid grid-cols-3 gap-2">
        <label className="space-y-1">
          <span className="text-[11px]" style={{ color: 'var(--ds-text-tertiary)' }}>{t.autoFormTrigger}</span>
          <select
            value={form.scheduleKind}
            onChange={(e) => update('scheduleKind', e.target.value as AutomationScheduleKind)}
            className="ds-input w-full px-3 py-2 text-xs rounded-lg"
          >
            <option value="manual">{t.autoFormManual}</option>
            <option value="cron">Cron</option>
            <option value="rrule">RRULE</option>
          </select>
        </label>
        <label className="space-y-1 col-span-2">
          <span className="text-[11px]" style={{ color: 'var(--ds-text-tertiary)' }}>{t.autoFormExpression}</span>
          <input
      ...
      <label className="space-y-1 block">
        <span className="text-[11px]" style={{ color: 'var(--ds-text-tertiary)' }}>{t.autoFormTimezone}</span>
        <input
          value={form.timezone}
...
function AutomationCard({
  automation, runs, running, onRun, onToggleStatus, onEdit, onDelete, onOpenSession,
}: {
  automation: Automation;
  runs: AutomationRun[];
  running: boolean;
    ...
    <div className="ds-card rounded-xl p-3 space-y-2 animate-fade-in">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[10px] px-1.5 py-0.5 rounded-full shrink-0" style={{ color: statusColor, background: statusBg }}>
              {automation.status === 'active' ? t.autoStatusActive : t.autoStatusPaused}
            </span>
          </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <IconButton
            title={automation.status === 'active' ? t.autoTogglePause : t.autoToggleResume}
            path={automation.status === 'active' ? 'M10 9v6m4-6v6' : 'M5 3l14 9-14 9V3z'}
          ...
          <IconButton
            title={t.delete}
            path="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 7V4a1 1 0 011-1h4a1 1 0 011 1v3m-9 0h12"
            onClick={onDelete}
            danger
          />
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        <Meta label={t.autoNext} value={formatTime(automation.nextRunAt)} />
        <Meta label={t.autoLast} value={formatTime(automation.lastRunAt)} />
        <Meta label={t.autoSession} value={automation.deepseek.chatSessionId ? shortId(automation.deepseek.chatSessionId) : t.autoNoSession} />
        <Meta label={t.autoRecent} value={latestRun ? formatRun(latestRun, t) : t.autoNone} />
      </div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        >
          {running ? t.autoRunning : t.autoRunNow}
        </button>
      </div>
    </div>
  );
...
function IconButton({ title, path, onClick, danger }: {
  title: string; path: string; onClick: () => void; danger?: boolean;
}) {
  return (
    <button
      title={title}
...
function formatRun(run: AutomationRun, t: ReturnType<typeof useLanguage>['t']): string {
  const label: Record<AutomationRun['status'], string> = {
    queued: t.autoRunQueued,
    running: t.autoRunning,
    succeeded: t.autoRunSucceeded,
    failed: t.autoRunFailed,
    timeout: t.autoRunTimeout,
    cancelled: t.autoRunCancelled,
    skipped: t.autoRunSkipped,
  };
  return `${label[run.status]}${run.attempt > 1 ? ` · ${run.attempt}x` : ''}`;
}
...
function normalizeFormModelType(modelType: string | null): string {
  if (!modelType || modelType === 'default' || modelType === 'DEFAULT' || modelType === 'chat' || modelType === 'deepseek_chat') return '';
